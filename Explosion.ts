        ...
        explode(SendFormData: FormData, x: number, y: number): void {
            ...
            for (let index: number = 0; index < SendFormData.amountParticle; index++) {
                ...
                switch (SendFormData.formParticle) {
                    case "dots": {
                        newElement = new Dots(SendFormData.color1, SendFormData.color2, SendFormData.formParticle, this.crc, x, y, SendFormData.drone);
                        break;
                    }

                    case "triangle": {
                        newElement = new Triangle(SendFormData.color1, SendFormData.color2, SendFormData.formParticle, this.crc, x, y, SendFormData.drone);
                        break;
                    }

                    case "line": {
                        newElement = new Line(SendFormData.color1, SendFormData.color2, SendFormData.formParticle, this.crc, x, y, SendFormData.drone);
                        break;
                    }
